  ...
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      className={cn(
        ...
        'rounded-[var(--radius-xl)] border border-base-600 bg-base-850 p-4 shadow-2xl sm:p-6',
    ...
    >
      {children}
      {!hideClose && (
        <DialogPrimitive.Close className="absolute right-5 top-5 rounded-md p-1 text-base-400 transition-colors hover:bg-base-700 hover:text-base-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet/50">
          <X className="size-4" />
...
const DialogFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn('flex flex-row justify-end gap-2 pt-5', className)} {...props} />
);
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Description
    ref={ref}
    className={cn('text-sm text-base-300', className)}
    {...props}
  />
));
